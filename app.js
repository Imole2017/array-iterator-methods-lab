...
let veryOldInventors = inventors.filter((inventor) => {
    if (1500 <= inventor.year && 1599 >= inventor.year)
    return inventor;
})
// OR 
// let veryOldInventors = inventors.filter((inventor)=> {
...
}); // a comes before b, so a.year is less than b.year, so if we consider that a.year is greater than b.year. per M mdn docs

// let sortedBirthYear = inventors.sort( inventor => ({birth: inventor.birth, year: inventor.year}))
// Check your work:
